...
import { 
  ...
  CalendarCheck, Users, FileText, BarChart3, Settings, Compass, Mail
} from 'lucide-react';
import { useApp } from '../../context/AppContext';
import ThemeToggle from './ThemeToggle';

export default function Navbar() {
  const { currentPage, navigateTo, currentUser, userRole, logout, login } = useApp();
  const [isOpen, setIsOpen] = useState(false);
  const [showProfileMenu, setShowProfileMenu] = useState(false);

  const isClientDashboard = currentPage.startsWith('client-') || currentPage === 'client-dashboard';
  const isAdminDashboard = currentPage.startsWith('admin-') || currentPage === 'admin-dashboard';
                ...
                {showProfileMenu && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl py-2 z-50 animate-slide-up">
                      ...
                      {/* Portal Shortcuts */}
                      {userRole === 'client' && !isClientDashboard && (
                        <button
                      ...
                      {userRole === 'admin' && !isAdminDashboard && (
                        <button
                          onClick={() => { handleNavClick('admin-dashboard'); setShowProfileMenu(false); }}
                          className="w-full text-left px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-800/60"
                        >
                          Go to Clinical Portal
                        </button>
                      )}
                      {(isClientDashboard || isAdminDashboard) && (
                        <button
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2.5 px-4 py-1.5">
                ...
                <div>
                  ...
                  <span className="block text-[9px] text-slate-400 capitalize">{userRole} Profile</span>
                </div>
              </div>
              ...
              {userRole === 'client' && !isClientDashboard && (
                <button
                  ...
                  className="block w-full text-left px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-300"
                >
                  Go to Patient Portal
                </button>
              )}
